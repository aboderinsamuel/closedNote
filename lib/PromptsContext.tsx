...
export function PromptsProvider({ children }: { children: React.ReactNode }) {
  ...
  // Optimistic helpers: callers mutate local state immediately so the UI feels
  // instant, then persist in the background. On failure, call refresh() to
  // re-sync with the database and undo the optimistic change.
